...
public class ParseXmlUtil {
    ...
    public ParseXmlUtil(String xmlFilePath) {
        try {
            ...
            for (Element req : reqElements) {
                reqConfig.put(req.attributeValue("name").trim(), StringUtils.isEmpty(req.getText().trim())?req.attributeValue("value").trim():req.getText().trim());
            }
            ...
            for (Element header : headerElements) {
                headerConfig.put(header.attributeValue("name").trim(), StringUtils.isEmpty(header.getText().trim())?header.attributeValue("value").trim():header.getText().trim());
            }
            ...
            for (Element dbcp : dbcpElements) {
                dbcpConfig.put(dbcp.attributeValue("name").trim(), StringUtils.isEmpty(dbcp.getText().trim())?dbcp.attributeValue("value").trim():dbcp.getText().trim());
            }
            ...
            for (Element excel : excelElements) {
                excelConfig.put(excel.attributeValue("name").trim(), StringUtils.isEmpty(excel.getText().trim())?excel.attributeValue("value").trim():excel.getText().trim());
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static void main(String[] args) {
        ParseXmlUtil parseXmlUtil = new ParseXmlUtil("./res/api-config.xml");
